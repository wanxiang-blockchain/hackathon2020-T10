...
class MqttServer():
    ...
    def csv_writer(self):
        headers = ['GpenID','StuName','StuIDCardNo','PoEHash']
        with open('csv_poe.csv','wb') as csvfile:
            f_csv = csv.DictWriter(csvfile, headers)
            f_csv.writeheader()


            if not all(k in self.get_value() for k in headers):
                return
            else:
                # f_csv.writerows(self.get_value())
                pass
...
if __name__ == '__main__':
    ...
    topics = ['/register/+/stu',
              ...
              '/exam/+/end']
    for i in range(0, 7):
        content = client.subscribe(topics[i], 0)
